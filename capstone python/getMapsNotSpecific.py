...
try:
	#with open(r"C:\Users\Wingwong\Desktop\outText.txt", 'r+') as inFile:
	for filename in list_of_texts:
		master_dict = dict()
		for line in open(indirectory + filename).readlines():
			if (line[6:len(line) - 1] not in types_of_words):
				...
				gotMatch = True
		bigger_master_dict[filename] = master_dict			
...
outFile = open(r"C:\Users\Wingwong\Desktop\outDict.txt", 'r+')

#for key in matched_dict:
	#outFile.write(str(key + '\t' + matched_dict[key] + '\n'))

with open(r"C:\Users\Wingwong\Desktop\outFinished.txt", 'w+') as outFile:
	for key in matched_dict:
		outLine = str(str(key) + '\t' + str(matched_dict[key]) + '\n')
		outFile.write(outLine)

unique_edges = 0
with open(r"C:\Users\Wingwong\Desktop\outFinished.txt", 'r+') as inFile:
	for line in inFile:
		unique_edges += 1
